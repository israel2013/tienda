...
<div class="form-group">
    ...
    <select class="form-select mb-3" v-model="ingreso.proveedor">
      <option value="" class="form-select mb-3">Seleccionar</option> 
...
<!--    
    ...
    <select class="form-select"  v-model="producto"  @change="producto_selected">
  
  <option v-for="option in productos" v-bind:value="option.value"  >
      {{ option.text }}
  </option>
...
<div class="form-group">
    ...
    <select class="form-select mb-3"  v-model="detalle.variedad">
      <option value="" class="form-select mb-3">Seleccionar</option> 
      <option :value="item._id" v-for="item in variedades">{{ item.variedad}}-{{ item.stok }}-{{ item.sku }}</option>
    </select>
...
<div class="col-md-12">
    ...
    <button class="btn btn-primary" style="margin-bottom: 1.8rem!important;" v-on:click="agregar_detalles()">
        Agregar
    </button>
</div>
</div> <!-- / .row -->
...
<div class="table-responsive mb-0">
    <table class="table table-sm table-nowrap card-table">
        ...
        <tbody class="fs-base" v-if="detalles.length
         >=1">
            ...
            <td>
                <time datetime="2020-04-24">
                  {{convertCurrency( item.precio_unidad) }}</time>
            </td>
            <td>
                {{ item.cantidad}}
            </td>
 ...
 //import { ModelSelect } from 'vue-search-select';
   ...
   export default {
    ...
    producto_selected(item){
      console.log(item);
      this.producto= item.target.value;
      //console.log(item.target.value);
      this.init_variedades(item.target.value);
     // console.log(item);
      this.detalle.producto=item.target.value;
      this.detalle.titulo_producto=item.target.value;//mostrar el nombre del producto y no el id
    },
